...
def hd95(pr, gt, eps=1e-7, threshold=None):
    ...
    for i in range(pr.shape[0]):
        v = np.array([[x, y] for x in range(m) for y in range(n) if pr[i, 0, x, y] > 0])
        u = np.array([[x, y] for x in range(m) for y in range(n) if gt[i, 0, x, y] > 0])
...
class Dataset(BaseDataset):
    ...
    def __getitem__(self, i):
        ...
        image = (image-(image**5/np.max(image**5))*255)**5
...
for epoch in range(1, n_epochs+1):
    ...
    # HD95_score_val = HD95_score_val.item()/len(valid_loader.dataset)
    ...
    print('Epoch: {}  Training Loss: {:.6f}  Validation Loss: {:.6f} Dice Score: {:.6f} IoU: {:.6f}'.format(
    ...
    with torch.no_grad():
        ...
        for data, target in bar:
            ...
            HD95_socore_test += hd95(output.cpu(), target.cpu())
    ...
    # HD95_score_test = HD_socore_test.item()/len(valid_loader.dataset)
    ...
    # print training/validation statistics
    print('Epoch: {}  Test Loss: {:.6f}  Dice Score: {:.6f} IoU: {:.6f}'.format(
...
with open('C:/Users/kuma/Dropbox/brain_mri/Script/result.json', 'w') as f:
